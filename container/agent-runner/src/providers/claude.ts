...
/**
 * Scan a Claude Code session transcript for tool_use blocks that have no
 * matching tool_result. For each dangling call, append a synthetic
 * tool_result placeholder so the model can resume without refusing.
 *
 * Reasoning models require a tool_result for every tool_use. A session
 * interrupted mid-tool-loop (container crash, nuke, compaction, timeout)
 * leaves the transcript in a state where the last assistant turn has
 * unresolved tool_use blocks. Without this repair, the resumed session
 * is "poisoned" — the model refuses to continue.
 */
export function repairDanglingToolCalls(transcriptPath: string): void {
  ...
  log(
    `Injecting ${dangling.length} placeholder tool_result(s) for interrupted tool call(s): ${dangling.join(', ')}`,
  );
  ...
  query(input: QueryInput): AgentQuery {
    ...
    if (input.continuation) {
      repairDanglingToolCalls(
        `/home/node/.claude/projects/${CLAUDE_PROJECT_SLUG}/${input.continuation}.jsonl`,
      );
    }
