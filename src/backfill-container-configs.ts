...
interface LegacyContainerJson {
  ...
  model?: string;
  effort?: string;
  // Fork-specific fields — preserved verbatim into the container_configs
  // `extensions` JSON column so the live agent keeps its capability sandbox,
  // observer, cost gate, etc. across the file→DB migration.
  isAdmin?: unknown;
  allowedCapabilities?: unknown;
  linkedinPostValidator?: unknown;
  loopDetection?: unknown;
  observer?: unknown;
  maintenanceSkillBlocklist?: unknown;
  progressiveSkills?: unknown;
  subagentLimit?: unknown;
  costGating?: unknown;
}

/** Keys carried verbatim from container.json into the `extensions` JSON column. */
...
export function backfillContainerConfigs(): void {
  ...
  for (const group of groups) {
    ...
    // Carry fork-specific fields verbatim into the extensions blob so they
    // survive the file→DB migration (capability sandbox, observer, etc.).
    const extensions: Record<string, unknown> = {};
    for (const key of EXTENSION_KEYS) {
      if (legacy[key] !== undefined) extensions[key] = legacy[key];
    }
