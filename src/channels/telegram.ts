...
registerChannelAdapter('telegram', {
  factory: () => {
    ...
    const telegramAdapter = createTelegramAdapter({
      botToken: token,
      mode: 'polling',
      // Pin allowed_updates so a stale server-side setting can't silently
      // drop callback_query (button clicks). Without this, the adapter
      // inherits whatever the last caller configured, and if that list
      // omits 'callback_query' every approval / ask_question card click
      // is dropped by Telegram before reaching the bot.
      longPolling: {
        allowedUpdates: [
          'message',
          'edited_message',
