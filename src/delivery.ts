...
/**
 * Per-message retry state. Resets on process restart (gives failed messages
 * a fresh chance). Backoff schedule on attempt N: 5s, 15s, 45s, 120s, 300s,
 * then permanent fail at attempt 6. Without backoff, the active 1s poll
 * burned through MAX_DELIVERY_ATTEMPTS in ~3 seconds, so any transient
 * channel blip longer than that lost messages permanently.
 */
