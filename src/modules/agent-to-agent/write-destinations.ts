 ...
 *
 * Security: only explicit `agent_destinations` rows are projected. Synthetic
 * peer/broadcast/main destinations were removed (LLM08 fix, issue #140) —
 * they allowed any agent to reach any other agent without authorization. Agent-
 * to-agent routes must now be explicitly wired (e.g. via `create_agent`, which
 * inserts bidirectional rows, or by operator-managed DB entries).
 */
import fs from 'fs';
