...
import type { McpServerConfig, OAuthConfig } from '../../container-config.js';
...
export const applyAddMcpServer: ApprovalHandler = async ({ session, payload, userId, notify }) => {
  ...
  // Add the new MCP server to the existing map in the DB. Supports both stdio
  // (command) and remote (url + optional OAuth) server shapes.
